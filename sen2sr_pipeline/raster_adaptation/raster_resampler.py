...
def resample(raster: xr.DataArray, resample_type: Resampling = Resampling.average, res: float = 2.5) -> xr.DataArray:
...
def resample_tif(tif_path: Path | str, res: float):
    """
    ...
    return resample(raster, Resampling.average, res)


def save_to_disk(resampled_raster: xr.DataArray, tif_path: Path | str, res: float):
    """
    Saves the resampled raster to the output directory.
    """
    file_name = f"{tif_path.stem}_{res}.tif"
    output_path = OUTPUT_PATH_BASE / file_name
